...
t_values <- function(t_value){
  beta_hat2 <- beta_hat + t_value * e_x2
  RSS <- (t((y - X %*% beta_hat2)) %*% (y - X %*% beta_hat2))
  response <- -(n / 2) * log(2 * pi * MLE) - (1 / (2* MLE)) * RSS
}

#Plug in every t value into the function
response_list <- list()
for (i in t){
  append(response_list, as.list(t_values(t[i])))
}

#beta_hat2 <- beta_hat + t %*% t(e_x2)
#reponse_values <- -(n / 2) * log(2 * pi * MLE) - (1 / (2* MLE)) * (t((y - X %*% beta_hat2)) %*% (y - X %*% beta_hat2))
```
