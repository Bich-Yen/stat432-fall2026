...
#Generate predictor and response values for scatterplot
##Variance is constant (MLE = MLE_hat)
t <- seq(from = -1.25, to = 1.25, length.out = 100)
beta_hat2 <- beta_hat + t %*% t(e_x2)
reponse_values <- -(n / 2) * log(2 * pi * MLE_hat) - (1 / (2* MLE_hat)) * (t((y - X %*% beta_hat2)) %*% (y - X %*% beta_hat2))
```
